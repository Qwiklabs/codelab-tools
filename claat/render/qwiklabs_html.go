...
func QwiklabsHTML(ctx Context, nodes ...nodes.Node) (htmlTemplate.HTML, error) {
	var buf bytes.Buffer
	if err := WriteQwiklabsHTML(&buf, ctx.Env, ctx.Format, nodes...); err != nil {
		return "", err
	}
	return htmlTemplate.HTML(buf.String()), nil
}

// WriteQwiklabsHTML does the same as HTML but outputs rendered markup to w.
func WriteQwiklabsHTML(w io.Writer, env string, fmt string, nodes ...nodes.Node) error {
	hw := qwiklabsHTMLWriter{w: w, env: env, format: fmt}
	return hw.write(nodes...)
}

// qwiklabs_ReplaceDoubleCurlyBracketsWithEntity replaces Double Curly Brackets with their charater entity.
func qwiklabs_ReplaceDoubleCurlyBracketsWithEntity(s string) string {
...
func (hw *qwiklabsHTMLWriter) matchEnv(v []string) bool {
	if len(v) == 0 || hw.env == "" {
		return true
	}
...
func (hw *qwiklabsHTMLWriter) write(nodesToWrite ...nodes.Node) error {
...
// Writes a string to the qwiklabsHTMLWriter unless a write error has occurred on this qwiklabsHTMLWriter in the past.
// Will set a write error on this qwiklabsHTMLWriter if the write fails.
func (hw *qwiklabsHTMLWriter) writeString(s string) {
	if hw.err != nil {
		return
	}
...
func (hw *qwiklabsHTMLWriter) writeFmt(f string, a ...interface{}) {
	hw.writeString(fmt.Sprintf(f, a...))
}

func qwiklabs_escape(s string) string {
	s = htmlTemplate.HTMLEscapeString(s)
	s = qwiklabs_ReplaceDoubleCurlyBracketsWithEntity(s)
	return s
}

// Same as writeString, but performs HTML escaping and double curly bracket escaping.
func (hw *qwiklabsHTMLWriter) writeEscape(s string) {
	hw.writeString(qwiklabs_escape(s))
}

func (hw *qwiklabsHTMLWriter) text(n *nodes.TextNode) {
	s := n.Value
	shouldEsc := true
	if n.Bold {
...
func (hw *qwiklabsHTMLWriter) image(n *nodes.ImageNode) {
	hw.writeString("<img")
	if n.Alt != "" {
...
func (hw *qwiklabsHTMLWriter) url(n *nodes.URLNode) {
	hw.writeString("<a")
	if n.URL != "" {
		hw.writeFmt(" href=%q", n.URL)
	}
	if n.Name != "" {
		hw.writeFmt(" name=%q", qwiklabs_escape(n.Name))
	}
	if n.Target != "" {
		hw.writeFmt(" target=%q", qwiklabs_escape(n.Target))
	}
	hw.writeString(">")
...
func (hw *qwiklabsHTMLWriter) button(n *nodes.ButtonNode) {
...
func (hw *qwiklabsHTMLWriter) code(n *nodes.CodeNode) {
	hw.writeString("<pre>")
	if !n.Term {
		hw.writeString("<code")
		if n.Lang != "" {
...
func (hw *qwiklabsHTMLWriter) list(n *nodes.ListNode) {
	wrap := n.Block() == true
	if wrap {
		if qwiklabs_onlyImages(n.Nodes...) {
...
func qwiklabs_onlyImages(nodesToCheck ...nodes.Node) bool {
...
func (hw *qwiklabsHTMLWriter) itemsList(n *nodes.ItemsListNode) {
	tag := "ul"
...
func (hw *qwiklabsHTMLWriter) grid(n *nodes.GridNode) {
	if hw.format == "devsite" {
...
func (hw *qwiklabsHTMLWriter) newBlock() {
	if !hw.lineStart {
		hw.writeString("\n")
	}
	hw.writeString("\n")
...
func (hw *qwiklabsHTMLWriter) infobox(n *nodes.InfoboxNode) {
	...
	hw.newBlock()
	hw.writeString("<div>") // for CommonMark compliance
	if n.Kind == nodes.InfoboxNegative {
		hw.writeString("<ql-warningbox>")
	} else {
...
func (hw *qwiklabsHTMLWriter) header(n *nodes.HeaderNode) {
...
func (hw *qwiklabsHTMLWriter) youtube(n *nodes.YouTubeNode) {
...
func (hw *qwiklabsHTMLWriter) iframe(n *nodes.IframeNode) {
